        ...
        <div class="container cuerpo">
           <div class="row">
               <div class="card col-lg-5 mt-5 mb-5 mx-auto">
                   <h2 class="text-center mt-3">User's info</h2>
                   <form action="servEditarUsers" method="post">
                       <input type="hidden" value="${idUsuario}" name="txtIdUsuario">
                       <div class="form-group">
                            <label>Name:</label>
                            <input  class="form-control" type="text" value="${nombre}" name="txtNombre" required>
                       </div>
                       <div class="form-group">
                            <label>Last name:</label>
                            <input  class="form-control" type="text" value="${apellido}" name="txtApellido" required>
                       </div>
                       <div class="form-group">
                            <label>E-mail:</label>
                            <input  class="form-control" type="email" value="${mailGlobant}" name="txtMail" required>
                       </div>
                       <div class="form-group">
                            <label>User's privilege:</label>
                            <select class="form-control" name="cboTipoUser" required>
                                <c:forEach var="ltu" items="${listaTU}">
                                    <option value="${ltu.idTipoUsuario}"<c:if test="${ltu.idTipoUsuario == idTipoUsuario}">selected</c:if>>${ltu.tipoUsuario}</option>
                                </c:forEach>
                            </select>
                       </div>
                       <div class="form-group">
                            <label>Initials: </label>
                            <input  class="form-control" type="text" value="${iniciales}" name="txtIniciales" required>
                       </div>
                       <div class="form-group">
                            <label>Asigned project:</label>
                            <select class="form-control" name="cboProyecto" required>
                                <c:forEach var="lp" items="${listaProyectos}">
                                    <option value="${lp.idProyecto}"<c:if test="${lp.idProyecto == idProyecto}">selected</c:if>>${lp.nombreProyecto}</option>
                                </c:forEach>
                            </select>
                       </div>
