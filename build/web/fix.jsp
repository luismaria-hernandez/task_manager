...
<html lang="en">
    ...
    <body>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">Fix Verification</h2></div>
                <div class="col" > 
                    <form class="form-inline float-right" method="GET">
                        <div class="form-group">
                            <label>Sprints:</label>
                            <select name="cboSprints" class="m-3 form-control" id="selOpcion">
                                <option value="">Select</option>
                                <c:forEach var="ls" items="${listaSprints}">
                                    <option value="${ls.idSprint}"<c:if test="${ls.idSprint == selSprint}">selected</c:if>>${ls.toString()}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <button type="submit" class="btn btn-primary" onclick="return validarSprints()">Show</button>
                    <form> 
                </div>
            </div>
           ...
           <table class="table table-hover table bordered" style="font-size: 1rem">
               <tr>
                   <th>Title</th><th>Ticket</th><th>Priority</th><th>Status</th><th>Admin Comment</th><th>Actions</th>
               </tr>
               <c:forEach var="lf" items="${listaFix}">
                   <tr><h5></h5></tr>
               <tr> 
                   <td>${lf.tituloFix}</td><td><a href="${lf.linkTicket}">${lf.linkTicket}</a></td><td><div class="tag bg-warning">${lf.prioridadFix}</div><td><div class="tag bg-warning">${lf.estado}</div></td><td>${lf.adminComment}</td><td><a href="servAbrirFix?id=${lf.idFix}" class="btn btn-primary">Run</a></td>
               </tr>
               </c:forEach>
           </table>
        </div>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">Fix Verification</h2></div>
                <div class="col" > 
                    <a href="servNuevoFix" class="btn btn-info mt-4 float-left"><b>NEW FIX + </b></a>
                    <form class="form-inline float-right m-3" method="GET">
                        <div class="form-group">
                            <label>Sprints:</label>
                            <select name="cboSprints" class="m-3 form-control" id="selOpcion">
                                <option value="">Select</option>
           ...
           <table class="table table-hover table bordered" style="font-size: 1rem">
               ...
               <c:forEach var="lf" items="${listaFix}">
                   <tr><h5></h5></tr>
               <tr> 
                   <td>${lf.tituloFix}</td><td><a href="${lf.linkTicket}">${lf.linkTicket}</a></td><td><div class="tag bg-warning">${lf.prioridadFix}</div><td><div class="tag bg-warning">${lf.estado}</div></td><td>${lf.adminComment}</td><td><a href="servAbrirFix?id=${lf.idFix}" class="btn btn-primary mr-4">Open</a><a href="servEditarFix?id=${lf.idFix}" class="mr-4">Edit</a><a href="servEliminarFix?id=${lf.idFix}" onclick="return eliminarFix()">Delete</a></td>
               </tr>
               </c:forEach>
           </table>
        </div>
