...
<html lang="en">
    ...
    <body>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">ADHOC</h2></div>
                <div class="col">
            ...
            <table class="table table-hover table bordered" style="font-size: 1rem">
                <tr>
                    <th>Title</th><th>Ticket</th><th>Status</th><th>Status Adhoc</th><th>Actions</th>
                </tr>
                <c:forEach var="la" items="${listaAdhoc}">
                <tr> 
                    <td>${la.tituloTicket}</td><td><a href="${la.linkTicket}">${la.linkTicket}</a></td><td><div class="tag bg-warning">${la.estado}</div></td><td><div class="tag bg-warning">${la.estadoAdhoc}</div></td><td><a href="servAbrirAdhoc?id=${la.idAdhoc}" class="btn btn-primary">Run</a></td>
                </tr>
                </c:forEach>
            </table>
        </div>
        ...
        <div class="container cuerpo">
            ...
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">ADHOC</h2></div>
                <div class="col">
                    <a href="servNuevoAdhoc" class="btn btn-info mt-4 float-left"><b>NEW ADHOC + </b></a>
                    <form class="form-inline float-right m-3" method="GET" action="">
                        <div class="form-group">
                            <label>Sprints:</label>
                            <select name="cboSprints" class="m-3 form-control" id="selOpcion">
                                <option value="">Select</option>
            ...
            <table class="table table-hover table bordered" style="font-size: 1rem">
                ...
                <c:forEach var="la" items="${listaAdhoc}">
                <tr> 
                    <td>${la.tituloTicket}</td><td><a href="${la.linkTicket}">${la.linkTicket}</a></td><td><div class="tag bg-warning">${la.estado}</div></td><td><div class="tag bg-warning">${la.estadoAdhoc}</div></td><td><a href="servAbrirAdhoc?id=${la.idAdhoc}" class="btn btn-primary mr-4">Open</a><a href="servEditarAdhoc?id=${la.idAdhoc}" class="mr-4">Edit</a><a href="servEliminarAdhoc?id=${la.idAdhoc}" onclick="return eliminarAdhoc()">Delete</a></td>
                </tr>
                </c:forEach>
            </table>
        </div>
