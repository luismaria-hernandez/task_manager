...
<html lang="en">
    <head>
        ...
        <title>DIRECTV'S TaskManager - Request</title>
    </head>
    <body>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">Requests</h2></div>
                <div class="col">
                    <table class="table table-hover table bordered" style="font-size: 1rem;">
                        <tr>
                            <th>Title</th><th>Ticket</th><th>Priority</th><th>Status</th><th>Fecha</th><th>Actions</th>
                        </tr>
                        <c:forEach var="lr" items="${listaRequests}">
                        <tr> 
                            <td>${lr.tituloRequest}</td><td><a href="${lr.linkTicket}">${lr.linkTicket}</a></td><td><div class="tag bg-warning">${lr.prioridadRequest}</div></td><td><div class="tag bg-warning">${lr.estado}</div></td><td>${lr.fecha}</td><td><a href="servAbrirRequest?id=${lr.idRequest}" class="btn btn-primary">Open</a></td>
                        </tr>
                        </c:forEach>
                    </table>
                </div>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">Requests</h2></div>
                <div class="col">
                    <a href="servNuevoReq" class="btn btn-info my-4 float-left"><b>NEW REQUEST + </b></a>
                    <table class="table table-hover table bordered" style="font-size: 1rem;">
                        <tr>
                            <th>Title</th><th>Ticket</th><th>Priority</th><th>Status</th><th>Fecha</th><th>Actions</th>
                        </tr>
                        <c:forEach var="lr" items="${listaRequests}">
                        <tr> 
                            <td>${lr.tituloRequest}</td><td><a href="${lr.linkTicket}">${lr.linkTicket}</a></td><td><div class="tag bg-warning">${lr.prioridadRequest}</div></td><td><div class="tag bg-warning">${lr.estado}</div></td><td>${lr.fecha}</td><td><a href="servAbrirRequest?id=${lr.idRequest}" class="btn btn-primary mx-2">Open</a><a href="servEditarReq?id=${lr.idRequest}" class="mx-2">Edit</a><a href="servEliminarReq?id=${lr.idRequest}" class="mx-2" onclick="return eliminarReq()">Delete</a></td>
                        </tr>
                        </c:forEach>
                    </table>
                </div>
