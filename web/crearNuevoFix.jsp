...
<html lang="en">
    ...
    <body>
        ...
        <div class="container cuerpo">
            <div class="row">
               <div class="card col-lg-5 mt-5 mx-auto mb-5">
                   <h2 class="text-center mt-3">Create a new Fix</h2>
                    <label>Projects:</label><br>
                    <form class="form-inline float-left" method="GET" action="servNuevoFix">
                            <div class="form-group">
                                <select name="cboProyectos" class="form-control mr-4" id="proyecto">
                                    <option value="">Select</option>
                                    <c:forEach var="lpr" items="${listaProyectos}">
                                        <option value="${lpr.idProyecto}"<c:if test ="${idProyecto == lpr.idProyecto}">selected</c:if>>${lpr.nombreProyecto}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        <button type="submit" class="btn btn-primary form-control" onclick="return validarCombos()">Select project</button>
                    </form> 
                   <div class="border-bottom mt-2 mb-3"></div>
                   <form action="" method="post">
                       <div class="form-group">
                            <label>Title:</label>
                            <input  class="form-control" type="text" name="txtTitulo"  id="titulo">
                       </div>
                       <div class="form-group">
                            <label>Ticket:</label>
                            <input  class="form-control" type="text" name="txtTicket"  id="ticket">
                       </div>
                       <div class="form-group"> 
                            <label>Priority:</label>
                            <select class="form-control" name="cboPrioridad"  id="prioridad">
                                <option value="">Select</option>
                                <c:forEach var="lp" items="${listaPrioridad}">
                                    <option value="${lp.idPrioridadFix}">${lp.prioridadFix}</option>
                                </c:forEach>
                            </select>
                       </div>
                       <div class="form-group">
                            <label>Status: </label>
                            <select class="form-control" name="cboEstado"  id="estado">
                                <option value="">Select</option>
                                <c:forEach var="les" items="${listaEstados}">
                                    <option value="${les.idEstado}">${les.estado}</option>
                                </c:forEach>
                            </select>
                       </div>
                       <div class="form-group">
                            <label>Sprint:</label>
                            <select class="form-control" name="cboSprints"  id="sprint">
                                <option value="">Select</option>
                                <c:forEach var="ls" items="${listaSprints}">
                                    <option value="${ls.idSprint}">${ls.toString()}</option>
                                </c:forEach>
                            </select>
                       </div>
                       <div class="form-group">
                            <label>Date:</label>
                            <input  class="form-control" type="date" name="dtpFecha"  id="fecha">
                       </div>
                       <div class="form-group">
                            <label>Admin comment:</label>
                            <textarea class="mt-3 form-control" rows="5" name="txtAdminComment" id="txtAdminComment" ></textarea>
                       </div>
                        <button type="submit" class="btn btn-primary form-control" onclick="return enviarNuevoFix()">Create</button>   
                    </form>
               </div>
           </div>
        </div>
