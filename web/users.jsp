...
<html lang="en">
    ...
    <body>
        ...
        <div class="container cuerpo">
            <div class="row">
                <div class="bg-dark text-white container-fluid"><h2 class="text-center p-3">Users</h2></div>
                <div class="col">
                    <a href="servAgregarUser" class="btn btn-info my-4 float-left"><b>NEW USER +</b></a>
                    <table class="table table-hover table bordered" style="font-size: 1rem">
                        <tr>
                            <th>Name</th><th>Last Name</th><th>E-mail</th><th>User's privilege</th><th>Initials</th><th>Asigned project</th><th>Actions</th>
                        </tr>
                        <c:forEach var="lu" items="${listaUsers}">
                        <tr> 
                            <td>${lu.nombre}</td><td>${lu.apellido}</td><td>${lu.mailGlobant}<td>${lu.tipoUsuario}</td><td>${lu.ini}</td><td>${lu.nombreProyecto}</td><td><a href="servEditarUsers?id=${lu.idUsuario}" class="btn btn-warning text text-white">Edit</a><a href="servEliminarUser?id=${lu.idUsuario}"class="btn btn-danger text text-white mx-2" onclick="return confirmarDelete()">Delete</a></td>
                        </tr>
                        </c:forEach>
                    </table>
